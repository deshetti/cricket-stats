...
class MenT20AllRounderRankingsSpider(scrapy.Spider):
    name = "menT20AllRounderRankings"

    def start_requests(self):
        start_date = date(2005, 2, 17)
        ...
        for i in range(int((end_date - start_date).days)+1):
            ...
            yield Request('http://www.relianceiccrankings.com/datespecific/t20/?stattype=all-rounder&day=%s&month=%s&year=%s' % (day,month,year), callback=self.parse)
    ...
    def parse(self, response):
        ...
        for i in range(2,len(rows)):
            ...
            iccid = rows[i].css('td.top100name > a::attr(href)').extract_first().replace('/playerdisplay/t20/all-rounder/?id=','')
