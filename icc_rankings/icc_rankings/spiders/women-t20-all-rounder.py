...
class WomenT20AllRounderRankingsSpider(scrapy.Spider):
    name = "womenT20AllRounderRankings"

    def start_requests(self):
        start_date = date(2004, 8, 5)
        ...
        for i in range(int((end_date - start_date).days)+1):
            ...
            yield Request('http://www.relianceiccrankings.com/datespecific/woment20/?stattype=all-rounder&day=%s&month=%s&year=%s' % (day,month,year), callback=self.parse)
    ...
    def parse(self, response):
        ...
        for i in range(2,len(rows)):
            ...
            rankingdate = response.css('#pagetitle > h2::text').extract_first().replace('ICC Twenty20 Championship All-Rounders - ','')
            iccid = rows[i].css('td.top100name > a::attr(href)').extract_first().replace('/playerdisplay/woment20/all-rounder/?id=','')
