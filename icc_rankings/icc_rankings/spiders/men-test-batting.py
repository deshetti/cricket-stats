...
class MenTestBattingRankingsSpider(scrapy.Spider):
    name = "menTestBattingRankings"
    ...
    def start_requests(self):
        ...
        for i in range(int((end_date - start_date).days)+1):
            ...
            yield Request('http://www.relianceiccrankings.com/datespecific/test/?stattype=batting&day=%s&month=%s&year=%s' % (day,month,year), callback=self.parse)
    ...
    def parse(self, response):
        ...
        for i in range(2,len(rows)):
            ...
            rankingdate = response.css('#pagetitle > h2::text').extract_first().replace('ICC Test Championship Batsmen - ','')
            iccid = rows[i].css('td.top100name > a::attr(href)').extract_first().replace('/playerdisplay/test/batting/?id=','')
