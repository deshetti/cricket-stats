...
class MenT20BowlingRankingsSpider(scrapy.Spider):
    name = "menT20BowlingRankings"
    ...
    def start_requests(self):
        ...
        for i in range(int((end_date - start_date).days)+1):
            ...
            yield Request('http://www.relianceiccrankings.com/datespecific/t20/?stattype=bowling&day=%s&month=%s&year=%s' % (day,month,year), callback=self.parse)
    ...
    def parse(self, response):
        ...
        for i in range(2,len(rows)):
            ...
            iccid = rows[i].css('td.top100name > a::attr(href)').extract_first().replace('/playerdisplay/t20/bowling/?id=','')
