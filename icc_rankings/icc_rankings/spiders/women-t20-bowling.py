...
class WomenT20BowlingRankingsSpider(scrapy.Spider):
    name = "womenT20BowlingRankings"
    ...
    def start_requests(self):
        ...
        for i in range(int((end_date - start_date).days)+1):
            ...
            yield Request('http://www.relianceiccrankings.com/datespecific/woment20/?stattype=bowling&day=%s&month=%s&year=%s' % (day,month,year), callback=self.parse)
    ...
    def parse(self, response):
        ...
        for i in range(2,len(rows)):
            ...
            rankingdate = response.css('#pagetitle > h2::text').extract_first().replace('ICC Twenty20 Championship Bowlers - ','')
            iccid = rows[i].css('td.top100name > a::attr(href)').extract_first().replace('/playerdisplay/woment20/bowling/?id=','')
