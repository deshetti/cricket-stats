...
class WomenODIBowlingRankingsSpider(scrapy.Spider):
    name = "womenODIBowlingRankings"

    def start_requests(self):
        start_date = date(1973, 6, 23)
        ...
        for i in range(int((end_date - start_date).days)+1):
            ...
            yield Request('http://www.relianceiccrankings.com/datespecific/womenodi/?stattype=bowling&day=%s&month=%s&year=%s' % (day,month,year), callback=self.parse)
    ...
    def parse(self, response):
        ...
        for i in range(2,len(rows)):
            ...
            rankingdate = response.css('#pagetitle > h2::text').extract_first().replace('ICC ODI Championship Bowlers - ','')
            iccid = rows[i].css('td.top100name > a::attr(href)').extract_first().replace('/playerdisplay/womenodi/bowling/?id=','')
